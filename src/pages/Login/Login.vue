  ...
  <section class="loginContainer">
    <div class="loginInner">
      <div class="login_header">
        <h2 class="login_logo">美食随行</h2>
        <div class="login_header_title">
          <a href="javascript:;" :class="{on: loginWay}" @click="loginWay=true">短信登录</a>
          <a href="javascript:;" :class="{on: !loginWay}" @click="loginWay=false">账号密码</a>
        </div>
      </div>
        ...
        <form @submit.prevent="login">
          <div :class="{on: loginWay}">
            ...
            <section class="login_hint">
              温馨提示：未注册美食随行帐号的手机号，登录时将自动注册，且代表已同意
  ...
  methods: {
    ...
    async login () {
      ...
      if(this.loginWay) {  // 短信登陆
        ...
        // 表单验证
        if(!this.name) {
          this.showAlert('未输入用户名')
          return
        } else if(!this.pwd) {
          this.showAlert('未输入密码')
          return
        } else if(!this.captcha) {
          this.showAlert('未输入验证码')
          return
        }
        // 发送ajax请求密码登陆
